...
from model.layers.att_layers import DenseAtt
...
def get_dim_act_curv(args):
    ...
    acts = [act] * (args.hgcn_num_layers - 1)

    dims = [args.hidden_size] + ([args.hidden_size*2] * (args.hgcn_num_layers - 1))

    dims += [args.hidden_size]
    acts += [act]
    n_curvatures = args.hgcn_num_layers
    if args.c is None:
...
class HypAgg(Module):
    ...
    def forward(self, x, adj):
        ...
        support_t = adj_att.unsqueeze(-1) * x_tangent
